...
> [!IMPORTANT]
> **第一优先级规范**：本设计系统与所有幻灯片生成代码中的色彩、组件与排版逻辑，必须以项目根目录下的 [UI_DESIGN_SPEC.md](file:///Users/lan/Rules_沉淀/deep-research/UI_DESIGN_SPEC.md) 以及 [UI_DESIGN_SPEC_yth.md](file:///Users/lan/Rules_沉淀/deep-research/cross-platform/UI_DESIGN_SPEC_yth.md) 规定的规范为**第一优先级**。任何其它备选方案及细节均不得违背上述规范。

## 配色方案参考 (Color Palette Reference)

在进行幻灯片输出时，必须根据汇报场景、受众和主题选择合适的配色体系。天机妙策局（deep-research）默认首推符合 UI 规范的**国网绿**配色方案。

| # | 配色名称 | 色值 (hex 无 #) | 风格 | 适用场景 | 说明 |
|---|---|---|---|---|---|
| 1 | **国网绿 (State Grid Green)** | `primary`: `"027C76"`<br>`secondary`: `"07978D"`<br>`accent`: `"0FB886"`<br>`light`: `"ebf7fa"`<br>`bg`: `"F6FBFC"` | 专业、稳健、双碳科技 | 国家电网相关汇报、能源电力行业、绿色发展、政企合作 | **核心品牌第一优先级配色**（源自 `UI_DESIGN_SPEC_yth.md` 规范）。用户提“国网绿”、国家电网或默认 PPT 渲染时，均自动激活此配色方案。 |
